...
describe("ghosttykit.error", function()
  ...
  it("maps spawn token errors", function()
    local err = errors.from_reply({ code = "spawn_token_not_found", error = "expired" })

    if not err then
      error("expected error response")
    end

    assert.are.equal("spawn_token_not_found", err.code)
    assert.are.equal("SpawnTokenNotFoundError", err.name)
  end)
